...
import { createSeasonsSchema, CreateSeasonsSchema, storeItemSchema, StoreSchema } from '@/validation/schema'
...
import { useCreateStoreItem } from '@/client_actions/superadmin/store'
  


export default function CreateStoreItems() {
    const [preview, setPreview] = useState<string | null>(null);
    const [open, setOpen] = useState(false)
    const {mutate: createStoreItem, isPending} = useCreateStoreItem()
  ...
  return (
    ...
    <DialogContent className=' max-w-[600px] h-auto border-amber-500/80 border-[1px] overflow-y-auto max-h-[80%]'>
      <DialogHeader className=' w-full bg-light p-3'>
        <DialogTitle className=' text-sm'>Create Store Items</DialogTitle>
        <DialogDescription>
