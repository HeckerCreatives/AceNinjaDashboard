...
export default function EditNewsForm(prop: Props) {
    ...
    const {mutate: editNews, isPending} = useEditNews()
    ...
    const editNewsDatas = async ( data: EditNewsData) => {
      editNews({newsid:prop.id,title: data.title, content: data.description, contentType: tab.toLowerCase(), url: data.file ?? ''},{
        onSuccess: () => {
  ...
  return (
    ...
    <DialogContent className=' max-w-[600px] h-auto max-h-[90%] border-amber-500/80 border-[1px]'>
      <DialogHeader className=' w-full bg-light p-3'>
        <DialogTitle className=' text-sm'>Edit News</DialogTitle>
        <DialogDescription>
