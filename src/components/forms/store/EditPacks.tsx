...
import { useUpdateFreebie, useUpdatePacks } from '@/client_actions/superadmin/freebie'
...
export default function EditPacks(prop: Props) {
  const [open, setOpen] = useState(false)
  const {mutate: updatePacks, isPending} = useUpdatePacks()
  ...
  return (
    ...
    <DialogContent className=' max-w-[600px] h-auto border-amber-500/80 border-[1px] overflow-y-auto max-h-[80%]'>
      <DialogHeader className=' w-full bg-light p-3'>
        <DialogTitle className=' text-sm'>Edit Packs</DialogTitle>
        <DialogDescription>
      ...
      <div  className=' text-xs flex flex-col gap-2 p-6 '>
            ...
            <div className=' w-full flex flex-col gap-1 p-4 bg-light rounded-md border-amber-800 border-[1px]'>
                ...
                <input value={price} onChange={(e) => setPrice(e.target.valueAsNumber)}   placeholder='Price' type='number' className={` input text-xs `}/>
            </div>
            ...
            {/* <div className=' w-full flex flex-col gap-1 p-4 bg-light rounded-md border-amber-800 border-[1px]'>
                 ...
                 <Select defaultValue={currencyType} onValueChange={setCurrencyType}>
            ...
            <div className=' w-full flex items-end justify-end gap-4 mt-6 text-white'>
                <button onClick={updatePacksData} disabled={isPending} className=' bg-yellow-500 text-black text-xs px-8 py-2 rounded-md flex items-center justify-center gap-1'>
                {isPending && <Loader/>}
                Save</button>
