...
import { useAddVipPack } from "@/client_actions/superadmin/vip-packs"
...
export default function AddPack() {
  ...
  const {mutate: addVipPack, isPending} = useAddVipPack()
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90%] h-fit w-full max-w-[700px] overflow-y-auto  p-6">
        <DialogHeader>
          <DialogTitle>Create VIP Pack</DialogTitle>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <Label>Name</Label>
              <Input {...register("name")}  className=" bg-zinc-900"/>
              {errors.name && <p className="text-red-500 text-sm">{errors.name.message}</p>}
            </div>

            <div>
              <Label>Amount</Label>
              <Input type="number" {...register("amount", { valueAsNumber: true })} className=" bg-zinc-900" />
                        ...
                        >
                          ...
                          <SelectContent>
                            {badgeItems?.data.map((item) => (
                              ...
                              key={item.id} value={String(item.index)}>
                                {item.title}
                              </SelectItem>
                            ))}
                          </SelectContent>
